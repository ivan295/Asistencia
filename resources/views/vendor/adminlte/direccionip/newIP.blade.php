...
<br>
<button type="button" class="btn btn-info" data-toggle="modal" data-target="#newip"><span
        class="glyphicon glyphicon-plus"></span> Agregar Nueva Dirección IP</button>
<form method="post" action="{{ route('direccion_ip.store')}}">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="modal fade" id="newip" tabindex="-1" data-backdrop="static" role="dialog" aria-labelledby="myModalLabel"
        ...
        <table id="datatable" class="table table-striped table-bordered">
            ...
            <tbody>
                ...
                <tr class='text-center'>
                    ...
                    <td>{{$dataip->departamento}}</td>
                    <td>{{$dataip->observacion}}</td>
                    <td class="text-center">
                        <div class="row">
                            <div class="col-md-3 col-md-offset-2">
                                <!-- <form action="{{ route('direccion_ip.edit',$dataip->id) }}" method="post">
                                {{csrf_field()}}
                                ...
                                <a class="btn btn-warning btn-xs" href="{{ route('direccion_ip.edit',$dataip->id) }}">Editar <i class="fa fa-edit"></i></a>
                            </div>
                            <div class="col-md-6 text-left">
                                <form class="form_eliminar" action="{{ route('direccion_ip.destroy',$dataip->id) }}" method="post"> 
                                    <input type="hidden" name="_method" value="DELETE">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    <button type="submit" class="btn btn-danger btn-xs">Borrar
